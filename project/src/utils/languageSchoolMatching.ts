...
export function calculateLanguageSchoolMatches(profile: ProfileFormData): LanguageSchoolMatch[] {
  ...
  const matches: LanguageSchoolMatch[] = languageSchools.map(school => {
    ...
    if (countryMatch && preferredCountries.length > 0) {
      matchScore += 25;
      pros.push(`Tercih ettiğiniz ülke: ${school.country}`);
    }
    ...
    if (estimatedTotalCost <= budget * 0.7) {
      pros.push(`Bütçenizin içinde (${estimatedTotalCost.toFixed(0)}$)`);
    } else if (estimatedTotalCost <= budget) {
      pros.push(`Bütçenize uygun (${estimatedTotalCost.toFixed(0)}$)`);
    } else {
      cons.push(`Bütçenizi aşıyor (${estimatedTotalCost.toFixed(0)}$)`);
    }
    ...
    if (school.visa_difficulty === 'easy') {
      ...
      cons.push('Vize süreci zorlu');
    }

    if (profile.visa_history?.has_rejections) {
      const rejectedCountries = profile.visa_history.rejections.map(r => r.country);
      if (rejectedCountries.includes(school.country)) {
        matchScore -= 30;
        cons.push('Daha önce bu ülkeden vize reddedildi');
      }
    }

    if (school.part_time_work_allowed) {
      matchScore += 10;
      pros.push('Part-time çalışma izni var');
    }

    if (school.pathway_programs) {
      matchScore += 10;
      pros.push('Üniversite pathway programı mevcut');
    }

    if (school.scholarship_available) {
      matchScore += 5;
      pros.push('Burs imkanı var');
    }
    ...
    if (school.rating >= 4.5) {
      pros.push(`Yüksek değerlendirme (${school.rating}/5)`);
    }

    if (school.class_size_max <= 12) {
      pros.push('Küçük sınıf mevcudu (daha fazla ilgi)');
      matchScore += 5;
    }
